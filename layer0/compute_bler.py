...
def compute_bler(use_case_number: int, factory_length_subdivision: int, factory_width_subdivision: int,
                 g_node_b_x: float, g_node_b_y: float, n_ues: int, payload: int):
    """
    Parameters
    --------
        use_case_number : int
    ...
    # Change the following parameter to use a different approach for UE distribution
    ...
    print(f"init tile and end tile are: {init_tile} and {end_tile}")
    # Convert the input use-case number to string
    # fixme: use-case 11 is not added here !!! , 12 added
    ...
    # Fixme: Add loop for changing machines tiles
    ...
    for each_loop in range(len(bler_dict)): # len(bler_dict) = total number of tiles -200 for test smaller loop
        ...
        # Fixme: here in Distribution I changed the number of machines on x , y to 1
        #  add another parameter for number of machines (machine_list_len) , machine height
        d = Distribution(use_case=use_case_input, factory_length=factory_length, factory_width=factory_width,
                         ...
                         inter_machine_distance=inter_machine_distance, machine_list_len= machine_list_len, machine_height=machine_height)

        # Instantiate machines, i.e. obstacles for the communication perspective
        # Distribute machines in the factory
        # now one machine is located in x[0],y[0] , Can I add a loop to change the position of the machine each time
        ...
        if run_machine_all_tiles:
            if inputs.get("geometry").get('machine_distribution') == 1:
                ...
                machine_array = [Machine(machine_size, d.get_ue_density(),machine_height) for i in range(len(machine_tile_x_ls))]
                d.distribute_selective_machines(machine_array, machine_tile_x_ls=machine_tile_x_ls, machine_tile_y_ls=machine_tile_y_ls)

            else:
                machine_array = [Machine(machine_size, d.get_ue_density(),machine_height) for i in range(d.get_number_of_machines())]
        ...
        if ue_distribution == 'tile_center':
            ...
            for index, ue in enumerate(ue_list):
                ...
                ue.set_coordinates(x_coordinate, y_coordinate, z_coordinate) # z values should consider fixed to machine size
            ...
            for rectangle_center in bler_dict.keys():
                ...
                if ue is not None:
                    ue.set_coordinates(random.uniform(rectangle_center[0]-rectangle_length/2, rectangle_center[0]+rectangle_length/2),
                                random.uniform(rectangle_center[1]-rectangle_width/2, rectangle_center[1]+rectangle_width/2),
                                random.uniform(0, max_machine_size))

            while ( ue := next((unset_ue for unset_ue in ue_list if unset_ue.get_coordinates() == (0.0, 0.0, 0.0)), None) ) is not None:
                ue.set_coordinates(random.uniform(0, factory_length), random.uniform(0, factory_width),
                                random.uniform(0, max_machine_size))
        else:
        ...
        g_node_b = GNodeB(params=inputs, n_channels=n_channels, ues_list=ue_list, starting_state=g_node_b_starting_state)
        ...
        for ue in ue_list:
            ...
            for iteration in range(n_iterations + 1):# n_iteration = 10000
                ...
                bit_error_probability = \
                    (modulation_order_ask - 1)/(modulation_order_ask * math.log2(modulation_order_ask)) * math.erfc(
        ...
        # remove indents the plot to run for one ue only
